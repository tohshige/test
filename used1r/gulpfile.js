...
gulp.task('csv2json2', function (callback) {
  ...
  //     console.log("complete item csv to JSON!!");
  //     callback();
  // });
  ...
  converter1.on('end_parsed', function (jsonArray) {
    ...
    console.log('complete select csv to JSON!!')
    callback()
  })
  require('fs').createReadStream('dist/dl-select201801290637-1.utf8.csv').pipe(converter1)
...
gulp.task('csvjson-help', function (callback) {
  var comment = 'check dist/dl-item2017xxxx.csv \n now convert to dist/all.utf8.json ! \n  end of line \n'
  console.log(comment)
  callback()
})
